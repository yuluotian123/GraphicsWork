    ...
    public class FFTOceanPass : ScriptableRenderPass
    {
        ...
        public void UpdateWaterSurface(CommandBuffer cmd,ScriptableRenderContext context, ref WaterSurfaceDatas surfaceData,ref WaterRenderingDatas renderData)
        {
            ...
            //泡沫纹理模糊
            {
                ...
                DualBoxBlur(cmd, context, textureDes, ref surfaceData.BubblesTexture, 2, 0.2f);
                DualBoxBlur(cmd, context,textureDes, ref renderData.BubblesSSSTexture, 3, 4.0f);
            }
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            //SetKeyWords
            {
                ...
                CoreUtils.SetKeyword(fftOceanMaterial, "IS_POSTPROCESSING", m_surfaceData.postProcessing);
            }
            ...
            fftOceanMaterial.SetTexture(m_renderingData._FoamMap,m_renderingData.FoamMap);
            fftOceanMaterial.SetTexture(m_renderingData._CausticsTexture, m_renderingData.CausticsTexture);

            //设置viewMatrix,因为blit过程不能正常获取V，P矩阵
            fftOceanMaterial.SetMatrix("_WaterCameraWorldToViewMatrix", renderingData.cameraData.GetViewMatrix());
            ...
            if (m_surfaceData.postProcessing)
            {
                //Blit做不到顶点偏移，所以考虑用DrawProcedural来在屏幕上打点。。。但是试了一下帧数很差而且效果不好
                var cameraColorTarget = renderingData.cameraData.renderer.cameraColorTarget;
                var tempDescriptor = renderingData.cameraData.cameraTargetDescriptor;
                ...
                cmd.Blit(cameraColorTarget, colorTemp.Identifier(), fftOceanMaterial, 1);
                //将临时缓冲区的结果存回源图像中
                cmd.Blit(colorTemp.Identifier(), cameraColorTarget);
            }
            else
            {
